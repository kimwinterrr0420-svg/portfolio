...
export const productDemo = [
  {
    ...
    overlay: {
      title: '播放页「边看边问」浮条怎么不打扰看剧',
      principle: '浮条不是常驻 UI，而是「暂停才出现」的二次入口：播放时完全隐形，只在用户暂停或主动唤起时浮现，用完即走。',
      states: [
        { name: '纯净态', desc: '播放中画面无任何 AI 元素，底部只有原生控制条' },
        { name: '唤起态', desc: '暂停视频时，「边看边问」浮条从底部滑出，答案严格限定在当前进度内' },
        { name: '收起态', desc: '点继续播放或点画面空白，浮条自动收回，不残留任何遮罩' }
      ]
    },
    form: {
  ...
  },
  {
    ...
    screens: [
      { key: 'player', label: '播放页' },
      { key: 'community', label: '社区 · 角色朋友圈' }
    ],
    overlay: {
      title: '播放页「角色气泡」的调起与隐藏',
      principle: '看视频是主任务，AI 不能抢戏：气泡默认折叠成右下角一枚半透明「AI」胶囊，不遮画面；用户主动需要（点胶囊 / 暂停）时才展开，用完自动收起。',
      states: [
        { name: '纯净态（默认）', desc: '播放中画面干净，仅右下角一枚半透明 AI 胶囊，角色气泡/边看边问全部隐藏' },
        { name: '唤起态', desc: '点 AI 胶囊或暂停时，角色气泡 + 边看边问浮条 + 一键二创依次浮现，此时视频已暂停，不遮挡关键画面' },
        { name: '收起态', desc: '点画面空白 / 继续播放 / 下滑手势，浮层自动收起回到纯净态，无残留遮罩' }
      ]
    },
    form: {
    ...
    interactions: [
      { text: '点播放页右下角「AI」胶囊（暂停时自动浮现）', sub: '唤起当前剧的角色气泡与边看边问，不遮挡画面' },
      { text: '与角色实时对话当前剧情', sub: '角色记得你追到第几集，自动规避剧透' },
      { text: '进入社区「角色朋友圈」，日常参与', sub: '点赞评论动态、看二创、买周边，关系可累积' }
    ]
  },
  {
    id: 'sol-gen',
    ...
    screens: [
      { key: 'mine', label: '我的 · 创作中心' }
    ],
    overlay: {
      title: '播放页「一键二创」按钮怎么不抢戏',
      principle: '「一键二创」收进播放页底部控制条，与暂停/倍速/弹幕并列，不单独占画面；点开后进入全屏创作面板，与观看场景完全隔离。',
      states: [
        { name: '常驻但低调', desc: '按钮收进底部控制条，仅在暂停或点开控制条时出现' },
        { name: '全屏创作', desc: '点开进入独立创作面板，不悬浮在画面上层' }
      ]
    },
    form: {
